...
const Detailsfirst = ({ navigation, route }) => {
    ...
    const handleSubmit = () => {
        if (
            ...
            const isValidPhone = /^(?:(?:(?:\+|00)?(91))[\s-]?)?(?:\d{10})$/.test(
                textInputValuePhone
            );
            if (!isValidPhone) {
                ...
                navigation.navigate("details2screen", {
                    state: selectedname,
                    address: textInputValueAddress,
                    city: textInputValueCity,
                    phoneno: textInputValuePhone,
                    pincode: textInputValuePin,
                });
